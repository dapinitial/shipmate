...
| *(before a ship)* | **Preflight** runs first, in seconds, no model: on the production branch, not behind origin, no protected file in the tree, guard installed, app spec matches the live app. It refuses *before* the grant is consumed, so the plan stays armed. And every clone carries shipmate's **pre-push guard** (`install-hooks.sh`), which refuses a push to any remote but the deploying one, a push that touches a protected path (secrets by default, plus the project's own list), or one that changes more files than the project's cap. `.shipmate.yml` in the repo holds those settings; the guard, preflight and the live check all read it. |
